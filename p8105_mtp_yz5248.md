...
The raw dataset was cleaned to ensure consistent and interpretable
variable formats.  
The key variables here is age, sex, eop size, eop visibility, and fhp
category.  
- Firstly, numeric variables coded as -999 were replaced with missing
values (NA).  
- The variable sex was recoded from numeric indicators (0, 1) to labeled
factors (“female”, “male”), and age_group was transformed into ordered
categorical groups (“18–30”, “31–40”, “41–50”, “51–60”, “60+”).  
- Categorical versions of eop size, eop visibility, and fhp category
were generated from their corresponding numeric or ordinal measures to
reflect the definitions described in the original report.

After cleaning, the final dataset included 1,221 participants, with
approximately equal numbers of males (607) and females (614). The age
distribution was relatively balanced across the predefined age groups,
with slightly fewer participants in the 60+ group. The table below
summarizes the number of participants by age and sex:
...
Age was categorized into five groups (18–30, 31–40, 41–50, 51–60, and
60+ years). The distribution across age groups is relatively uniform,
with about 30% of participants aged 18–30, 20% each in the 31–60 range,
and roughly 10% aged 60 and above for both sexes. This balanced sampling
ensures adequate representation across adult age groups.
...
```
    ...
    ## # A tibble: 8 × 5
    ##   sex      age fhp_size_mm fhp_category       fhp_cat_from_size 
    ##   <fct>  <dbl>       <dbl> <ord>              <ord>             
    ## 1 female    64        21.1 mild (10–20mm)     moderate (20–30mm)
    ## 2 male      29        33.8 mild (10–20mm)     severe (30mm+)    
    ## 3 male      42        22.9 mild (10–20mm)     moderate (20–30mm)
    ## 4 male      18        31.8 moderate (20–30mm) severe (30mm+)    
    ## 5 male      65        30.1 mild (10–20mm)     severe (30mm+)    
    ## 6 female    63        31.5 moderate (20–30mm) severe (30mm+)    
    ## 7 female    28        27   mild (10–20mm)     moderate (20–30mm)
    ## 8 male      65        25   mild (10–20mm)     moderate (20–30mm)

Comparison between fhp_category and the continuous fhp_size_mm values
reveals several inconsistencies. For example, subjects with FHP sizes of
21–25 mm are sometimes labeled as mild (10–20 mm) rather than moderate
(20–30 mm), and those with values above 30 mm are occasionally
categorized as mild or moderate instead of severe (30 mm+). These
discrepancies indicate that the categorical variable fhp_category does
not strictly follow the definitions implied by the underlying continuous
measurements. Such coding errors could distort analyses comparing FHP
severity across demographic groups or relating FHP to other anatomical
features.
...
About the left panel shows that the full distribution of fhp size,
revealing right-skewed patterns and larger fhp in males and older
participants—details hidden in the original summary statistics. About
the right panel, the plot of enlarge eop_size shows that the prevalence
of enlarged eop increases with age in both males and females, with males
generally having a higher prevalence than females across all age
groups.  
Overall, these figures highlight sex and age differences but do not
support a causal link between smartphone use and eop growth; additional
data on posture and device use would be needed.
...
![](p8105_mtp_yz5248_files/figure-gfm/unnamed-chunk-6-1.png)<!-- --> The
plots show a weak positive association between fhp and eop size in males
across most age groups, while females display little or no clear
relationship. Males generally have larger eop sizes than females, and
age does not appear to substantially affect the association. Overall,
sex differences, rather than age, mainly drive the relationship between
fhp and eop.
